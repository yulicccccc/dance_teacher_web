...
| 范围 | 已落实行为 | 回归护栏 |
|---|---|---|
| P0-1~4 | 仅一个固定文案“循环”按钮；按钮只开关；单节/多节/AB 三选一；多节空选置灰 | `ControlBar.loopButton.test.tsx` |
| P0-5~7 | 删除重复 `LoopPanel`；多节勾选只在左栏出现；单节/AB 不出现勾选框 | 全局 `LoopPanel` 搜索为空；`SegmentList.multiSelect.test.tsx` |
...
并在正式 URL 用真实 MOV 验证上传、BPM/置信度、三种循环和手机端播放。
